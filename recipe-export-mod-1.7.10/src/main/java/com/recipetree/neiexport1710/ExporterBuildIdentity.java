...
final class ExporterBuildIdentity {
    ...
    static final String EXPORTER_ID = "forge-nei-gtnh-1.7.10";
    static final String MINECRAFT_VERSION = "1.7.10";
    ...
    static ExporterBuildIdentity loadRuntime() throws IOException {
        ModContainer container = Loader.instance().getIndexedModList().get(GtnhNeiExportMod.MOD_ID);
        if (container == null) {
            throw new IOException("Exporter mod container is unavailable; build provenance cannot be resolved");
        }
        File source = container.getSource();
        if (source == null) {
            throw new IOException("Exporter mod container has no source JAR; no provenance fallback was attempted");
        }
        return readAndVerify(source.toPath());
    }
    ...
    static ExporterBuildIdentity readAndVerify(Path jarPath) throws IOException {
        ...
        try (JarFile jar = new JarFile(normalized.toFile(), false)) {
            List<JarEntry> payloadEntries = new ArrayList<JarEntry>();
    ...
    private static String digestPayload(JarFile jar, List<JarEntry> entries) throws IOException {
        Collections.sort(entries, new Comparator<JarEntry>() {
            ...
            public int compare(JarEntry left, JarEntry right) {
                return compareUtf8(left.getName(), right.getName());
            }
        });
