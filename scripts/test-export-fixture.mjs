...
export async function writeNonUniformImage(path) {
...
export async function createRawExportFixture(root) {
  await mkdir(join(root, 'icons'), {recursive: true});
  await mkdir(join(root, 'recipes', 'minecraft_crafting'), {recursive: true});
  await writeNonUniformImage(join(root, 'icons', 'stone.png'));
  await writeNonUniformImage(join(root, 'recipes', 'minecraft_crafting', 'icon.png'));
